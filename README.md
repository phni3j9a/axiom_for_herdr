...
結果待ちの間隔も実行環境・応答性のルールで許される長さ（1分が上限なら60秒）に
明示設定するよう指示します。短い間隔での結果取得や、進捗を見るためだけの
`status`・端末ログの反復取得を避けます。
...
Mainの起床回数をゼロにする仕組みではなく、実行ツールの上限やユーザーへの応答性に
従って確認頻度を抑える方針です。コンテキストやツール結果を失った場合は、`status`で
