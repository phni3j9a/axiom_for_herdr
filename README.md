...
| 作業待ち | 1つの待機プロセスで全担当を監視。Mainは同じ実行セッションを長めの間隔で待つ |
...
内部の2秒ごとの確認ではMainのLLMを呼びません。Mainには同じ実行セッションを保持し、
**結果待ちを5分（300秒、`yield_time_ms=300000`）固定**にするよう指示します。
「環境で許される最長間隔」という裁量は設けず、Mainが自己判断で1分などに短縮することを
禁止します。外側の実行ラッパーが待機を返す場合も同じ値を使い、進捗を見るためだけの
`status`・端末ログの反復取得は行いません。完了・要対応・プロセス終了・ユーザーの追加指示で
早く戻った場合は、その時点で対応します。5分経つまで結果を保留する設定ではありません。
...
補助スクリプトの待機時間と、Codexが実行結果を待つ時間は別設定です。
実行ツールや上位ルールが5分待機に対応しない場合は、その制約を一度報告し、
短いポーリングへ自動で読み替えません。既存の待機・担当を保持し、対応済みのイベント待機か
独立した作業へ移ります。スキルの指示だけでホストの上限を変更・強制することはできません。
